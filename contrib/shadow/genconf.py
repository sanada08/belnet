...
libpath = 'libshadow-plugin-belnet.so'
...
def genconf(settings, outf):
    ...
    svcNodeCount = getSetting(settings, 'master-nodes', 80)
    peers = list()
    ...
    # make all master nodes know each other
    for peer in peers:
        for nodeid in range(svcNodeCount):
...
if __name__ == '__main__':
    settings = {
        'baseDir': os.path.join("/tmp", "belnet-shadow"),
