...
main(int argc, char* argv[])
{
  ...
  const char* confFile = belnet::bootserv::Config::DefaultPath;
  belnet::bootserv::Config config;

  belnet::bootserv::Handler_ptr handler;
  ...
  if(RunCron)
    handler = belnet::bootserv::NewCronHandler(std::cout);
  else
    handler = belnet::bootserv::NewCGIHandler(std::cout);
